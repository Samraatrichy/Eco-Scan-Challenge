...
# Function to change the eco tip every 5 seconds
def eco_tip():
    while True:
...
#load_dotenv()
OPENAI_API_KEY="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"
...
st.markdown("""
    <style>
        ...
        .main-title {
            font-size: 60px; /* Increase title size */
            font-weight: bold;
...
st.markdown(
    ...
    <style>
        ...
        [data-testid="stSidebar"] {
            background-color: #040700; /* Replace with your desired color */
            color: white; /* Optional: Adjust text color */
        }

        /* Optional: Change the font style and padding in the sidebar */
...
# Tabs for navigation
tab1, tab2, tab3, tab4 = st.tabs(["Home", "About", "Scan", "Quiz"])
...
with tab1:
    ...
    <style>
        ...
        .features-container {
            ...
            gap: 60px; /* Increased gap for more spacing between feature cards */
            padding: 60px 20px;
        }
        .feature-card {
            background-color: #CCE8C2;
            border-radius: 12px;
            box-shadow: 0 10px 20px rgba(0, 0, 0, 0.2); /* Increased shadow distance */
            padding: 30px;
            text-align: center;
        ...
        .feature-card:hover {
            ...
            box-shadow: 0 15px 30px rgba(0, 0, 0, 0.3); /* Enhanced shadow effect */
        }
        .feature-card img {
            width: 80px;
            height: 80px;
...
st.markdown("""
    <style>
        .footer {
            ...
            background-color: #b9cba2; /* Light green background */
    ...
    <div class="footer">
        EcoScan - Join us in reducing the fashion industry’s carbon footprint. 
        Make smarter, eco-friendly choices!
    </div>
""", unsafe_allow_html=True)
...
# Tab 3: Upload
with tab3:
    st.markdown("### Scan Your Clothing Image Here")
    ...
    with col1:
        uploaded_file = st.file_uploader("Upload Image", type=["jpg", "jpeg", "png","PSD","SVG","EPS","PDF","RAW","HEIC","WEBP"])
        if uploaded_file:
    ...
    with col2:
        ...
        if st.session_state.camera_enabled:
            toggle_text = "🚫 Turn Off Camera"
        else:
            toggle_text = "📸 Turn On Camera"

        camera_toggle = st.button(toggle_text, key="camera_toggle")

        if camera_toggle:
            # Toggle the camera state
            st.session_state.camera_enabled = not st.session_state.camera_enabled
            st.query_params = {"camera": st.session_state.camera_enabled}  # Update query parameters

        if st.session_state.camera_enabled:
            st.info(
                """
                **Guidance:** 
                - Ensure your clothing item is well-lit and centered in the camera frame.  
                - Hold steady for a clear picture to improve analysis accuracy.  
                - Click the button below to capture your image.  
                """
            )
            with st.expander("📸 Capture your clothing image", expanded=True):
                st.write("Please position the camera to capture the image of the clothing item.")
                camera_photo = st.camera_input("Take a picture")

                if camera_photo:
                    st.success("Image captured successfully! You can proceed with the analysis.")

    # Customize file uploader styling with modern green color
    st.markdown("""
    <style>
        .stFileUploader {
            background-color: #linear-gradient(135deg, #006400, #4CAF50);
            border: 2px solid #4CAF50;
            padding: 20px;
            border-radius: 12px;
        ...
        .stButton button {
            ...
            color: white;
            padding: 15px 35px; /* Increased button size */
            font-size: 18px;
            border-radius: 10px;
            font-weight: bold;
            border: none;
        ...
        .stButton button:hover {
            background-color: #249e06; /* Lighter green on hover */
        }
        ...
        /* Eco tip positioning and styling */
        .eco-tip {
            margin-top: 40px;
            text-align: center;
            background-color: #e7f7ec;
            ...
            font-size: 16px;
            font-weight: bold;
            width: 100%;  /* Center the eco tip */
        }
    </style>
    """, unsafe_allow_html=True)

    # Centered Scan Image button above Eco Tip
    st.markdown('<div style="text-align: center; margin-bottom: 20px;">', unsafe_allow_html=True)
    analyze_button = st.button("🔍 Scan Image", key="analyze", help="Click to analyze the image")
    st.markdown('</div>', unsafe_allow_html=True)

    # Display short info message after clicking the "Scan Image" button
    if analyze_button:
        # Ensure that input_image is set correctly based on file upload or camera input
        input_image = uploaded_file if uploaded_file else (camera_photo if 'camera_photo' in locals() and camera_photo else None)
        
        if input_image is None:
            st.info("Please upload or capture an image of the clothing item(s).", icon="ℹ️")
        else:
            st.info("Analyzing your image...", icon="ℹ️")
            
            # Continue with processing the image...
            temp_image_path = "input_image.png"
            with open(temp_image_path, "wb") as f:
                f.write(input_image.getbuffer() if uploaded_file else input_image.getvalue())  # Handle file vs camera input
    ...
    # Eco tip positioned at the bottom of all content
    st.markdown('<div class="eco-tip">Remember, the more sustainable your choices, the better for our planet! 🌍</div>', unsafe_allow_html=True)
